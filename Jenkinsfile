pipeline {
  ...
  stages {
    ...
    stage('Deploy to EKS') {
      steps {
        // Inject the kubeconfig file as an environment var KUBECONFIG
        withCredentials([file(
          credentialsId: 'eks-kubeconfig',
          variable: 'KUBECONFIG'
        )]) {
          sh """
            # Use direct --kubeconfig instead of withKubeConfig
            kubectl --kubeconfig=${KUBECONFIG} -n default set image deployment/orders-api \
              orders-api=${ECR_REPO}:${IMAGE_TAG}

            kubectl --kubeconfig=${KUBECONFIG} -n default rollout status deployment/orders-api
          """
        }
      }
    }
  ...
  post {
    ...
    failure { echo "❌ Pipeline failed—check console logs for details." }
  }
}
